...
export default function Top10Page() {
  ...
  const { data: rawRestaurants, isLoading } = useGetTopRestaurants(
    { mode },
    { query: { queryKey: ["topRestaurants", mode] } }
  );

  const restaurants = useMemo(() => {
    if (!rawRestaurants) return [];
    let filtered = [...rawRestaurants];
    if (selectedArea !== "All Areas") {
      filtered = filtered.filter((r) => r.area === selectedArea);
    }
    if (selectedCerts.length > 0) {
      filtered = filtered.filter((r) =>
        selectedCerts.every((c) => r.certifications?.includes(c))
      );
    }
  ...
  const displayScore = (r: typeof restaurants[0]) =>
    mode === "kashif" ? (r.kashif_score as number | null) : (r.avg_score as number | null);
  ...
  return (
    <div className="page-content">
      ...
      >
        ...
        <p style={{ color: "#7A6A58", fontSize: "0.82rem", margin: "0 0 16px" }}>
        ...
        <div style={{ display: "flex", gap: 8, width: "max-content" }}>
          {CERTS.map((cert) => (
            <button
              ...
              style={CERT_STYLE(selectedCerts.includes(cert))}
            >
              {cert}
            </button>
          ))}
      ...
      <div style={{ padding: "16px 20px" }}>
        {isLoading && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} style={{ height: 76, background: "#F0E8DC", borderRadius: 14 }} />
            ))}
          </div>
        )}
        ...
        {!isLoading && restaurants.length === 0 && (
          <div style={{ textAlign: "center", padding: "48px 0" }}>
            ...
            <p style={{ color: "#7A6A58", fontSize: "0.9rem" }}>
              {selectedCerts.length > 0 || selectedArea !== "All Areas"
                ? "Try adjusting your filters."
                : mode === "kashif"
                ? "Kashif hasn't dropped noms yet — check back soon."
                ...
                <Link key={r.id} href={`/restaurant/${r.id}`}>
                  ...
                  >
                          ...
                          {r.certifications.map((c) => (
                            <span
                              ...
                              style={{
                                ...
                                background: c === "Muslim-Owned" ? "#2C1E0C" : "#7A4800",
                                color: c === "Muslim-Owned" ? "#D4C8BC" : "#FFD166",
                                padding: "1px 6px",
                                borderRadius: 99,
                              }}
                            >
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      {score != null ? (
                        <>
                          <div style={{ fontFamily: "var(--app-font-display)", fontSize: "1.8rem", color: "#E8420A", letterSpacing: "0.04em", lineHeight: 1 }}>
                            {score.toFixed(1)}
                          </div>
                          <div style={{ fontSize: "0.58rem", color: "#7A6A58", fontWeight: 600, letterSpacing: "0.08em" }}>
                            {mode === "kashif" ? "KASHIF" : "/10"}
                          </div>
                        </>
                      ) : (
